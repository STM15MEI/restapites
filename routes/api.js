...
router.get('/downloader/youtube-play-audio', async (req, res, next) => {
  ...
  xc = await ytmp3(`https://www.youtube.com/watch?v=${id}`)
  xs = await convertToUrl(xc.buffer)
  res.json({
    status: "Success",
    code: 200,
    author: author,
    data: {
      title: xc.meta.title,
      channel: xc.meta.channel,
      duration: xc.meta.seconds,
      thumb: xc.meta.image,
      url: xs
    }
  })
...
router.get('/downloader/youtube-play-video', async (req, res, next) => {
  ...
  let search = await yts(query)
  id = search.videos[0].videoId;
  xc = await ytmp4(`https://www.youtube.com/watch?v=${id}`)
  xs = await convertToUrl(xc.buffer)
  res.json({
    status: "Success",
    code: 200,
...
router.get('/downloader/tiktok', async (req, res, next) => {
  ...
  xcode = await fetchJson(`https://api.botcahx.eu.org/api/dowloader/tiktok?url=${url}&apikey=IchanRTZ`)
  let result = xcode.result
  if (!result) res.json(msg.nodata)
  res.json({
    status: "Success",
...
router.get("/others/toanime", async (req, res, next) => {
  ...
  const url = req.query.url
  if (!url) return res.json(msg.paramurl)
  await getBuffer(`https://api.lolhuman.xyz/api/imagetoanime?apikey=SGWN&img=${url}`).then(buffer => {
    res.set({ 'Content-Type': 'image/png' })
    res.send(buffer)
  })
  limitMin(apikey);
